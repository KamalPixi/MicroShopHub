    ...
    <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-lg mx-auto mt-10">
        ...
        <form action="{{ route('admin.password.email') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email') }}" required autofocus class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm placeholder:text-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none" placeholder="admin@example.com">
                <p class="mt-2 text-xs text-gray-500">You can request a reset link a few times before the system pauses new requests.</p>
            </div>
