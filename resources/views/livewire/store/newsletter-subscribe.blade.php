<div>
    <form wire:submit.prevent="subscribe" class="max-w-md mx-auto flex flex-col sm:flex-row gap-3">
        <input type="email" wire:model.live="email" required placeholder="{{ __('store.enter_your_email') }}" class="flex-1 px-4 py-3 rounded-lg text-gray-900 focus:outline-none">
        <button type="submit" class="bg-white text-primary px-6 py-3 rounded-lg font-semibold hover:bg-gray-100">{{ __('store.subscribe') }}</button>
    </form>
    @error('email')
