            ...
            <div class="md:col-span-1">
                <div class="flex items-center gap-3">
                    ...
                    <div class="leading-tight">
                        ...
                        @if(!empty($storeSlogan))
                            <p class="mt-0.5 text-xs leading-tight uppercase tracking-[0.2em] text-primary/70">{{ $storeSlogan ?: __('store.shop_slogan_default') }}</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="mt-5 flex items-center gap-3">
                    @if(!empty($footerSettings['footer_social_facebook_url'] ?? ''))
                        <a href="{{ $normalizeSocialUrl($footerSettings['footer_social_facebook_url']) }}" target="_blank" rel="noopener noreferrer" class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-gray-200 text-gray-500 hover:border-primary/25 hover:text-primary" aria-label="Facebook">
                    ...
                    @if(!empty($footerSettings['footer_social_x_url'] ?? ''))
                        <a href="{{ $normalizeSocialUrl($footerSettings['footer_social_x_url']) }}" target="_blank" rel="noopener noreferrer" class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-gray-200 text-gray-500 hover:border-primary/25 hover:text-primary" aria-label="X">
                    ...
                    @if(!empty($footerSettings['footer_social_instagram_url'] ?? ''))
                        <a href="{{ $normalizeSocialUrl($footerSettings['footer_social_instagram_url']) }}" target="_blank" rel="noopener noreferrer" class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-gray-200 text-gray-500 hover:border-primary/25 hover:text-primary" aria-label="Instagram">
