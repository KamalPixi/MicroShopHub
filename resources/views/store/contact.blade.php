    ...
    <div class="space-y-4 py-6 md:py-8">
        <div class="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
            <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                <p class="text-xs font-semibold uppercase tracking-[0.24em] text-primary">{{ __('store.support') }}</p>
                <h1 class="mt-2 text-3xl font-extrabold text-gray-900">{{ __('store.contact_us') }}</h1>
                <p class="mt-3 text-sm leading-6 text-gray-600">{{ __('store.support_intro') }}</p>

                <div class="mt-6 space-y-3 text-sm">
                    <div class="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                        <p class="text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">{{ __('store.shop_email') }}</p>
                        <p class="mt-1 font-medium text-gray-900">{{ $supportEmail ?? 'support@shophub.com' }}</p>
                    </div>
                    <div class="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                        <p class="text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">{{ __('store.phone_number') }}</p>
